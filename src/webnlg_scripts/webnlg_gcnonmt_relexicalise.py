...
from webnlg_gcnonmt_input import input_files, SEEN_CATEGORIES
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, 'i:f:p:c:g:l',
                                   ['inputdir=', 'filedir=', 'partition=', 'category=', 'gpuid=', 'lowercased='])
    ...
    if partition:
        if category == 'seen':
            input_files(inputdir, filepath, relex=True, parts=[partition], doCategory=SEEN_CATEGORIES, fileid=gpuid,
